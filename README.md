...
## OpenClaw and Exposing the Tools

This repo does not ship an OpenClaw plugin or HTTP API. To use the same *design* and *tool contracts* inside [OpenClaw](https://docs.openclaw.ai/):

1. **Skill**: The `SKILL.md` (AgentSkills/OpenClaw-compatible) that describes the wallet model, security considerations, and the tool names/parameters/behavior. Use it to implement an OpenClaw skill or tool that talks to your own backend (e.g. a small HTTP API that wraps this app’s `WalletService` / `GeminiAgentService` logic).
2. **Implement tools on your side**: Expose endpoints or a CLI that map to the same tools (e.g. `create_agent`, `get_balance`, `transfer_sol`, `request_airdrop`, `get_assets_by_owner`, `buy_token`). Keep the same security rules (master key, max_spend, simulation).
3. **Register the skill**: Put the skill in OpenClaw’s workspace `skills/` or in `~/.openclaw/skills/` and, if you add a plugin that implements the tools, wire the skill to that plugin so the agent can call them.

See `SKILL.md` for the tool schemas, security notes, and how an agent should use them (including confirmation and max_spend). That file is the reference for producing an OpenClaw-ready skill or SKILLs index.
